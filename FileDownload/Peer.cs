...
public class Peer {
    ...
    public async Task DownloadFile(String peerIP, int peerPort, string fileName, string savePath, CancellationToken cancellationToken) {
        ...
        Console.WriteLine($"El archivo {fileName} de ha descargado en la ruta {savePath}");
    }
    ...
    public async Task HandleClient(CancellationToken cancellationToken) {
        ...
        if (File.Exists(fileName)) {
            ...
            Console.WriteLine($"El archivo {fileName} fue enviado al cliente");
        } else {
            var errorMessage = Encoding.UTF8.GetBytes("No se encontro el archivo");
            await stream.WriteAsync(errorMessage, cancellationToken);
            Console.WriteLine($"No se encontro el archivo {fileName}");
        }   
    }
}